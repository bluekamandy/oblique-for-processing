...
void main()
{
    ...
    highp float redTextureColor = step(.5, texture2D(srcTex, vec2(vertTexCoord.x - remappedX, vertTexCoord.y - remappedY)).r);
    highp float greenTextureColor = step(.5, texture2D(srcTex, vertTexCoord.xy).g);
    highp float blueTextureColor = step(.5, texture2D(srcTex, vec2(vertTexCoord.x + remappedX, vertTexCoord.y + remappedY)).b);
